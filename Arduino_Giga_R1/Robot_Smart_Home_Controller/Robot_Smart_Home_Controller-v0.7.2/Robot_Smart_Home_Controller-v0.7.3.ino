  ...
  for (page_nr=0; page_nr < max_info_pages; page_nr++) {
    ...
    switch (page_nr) {
      ...
      case PAGE_IMU_BNO055_ID:                 //  5
       ...
       break;
      case PAGE_IMU_ID:               //  6
        Serial.print(PAGE_IMU_TITLE);

        pages[page_nr].name = PAGE_IMU_NAME;
        pages[page_nr].title = PAGE_IMU_TITLE;
        ...
        switch (page_id) {
          ...
          case PAGE_IMU_ID:
            if ((USING_LSM6DSOX_9DOF and sensor_status.lsm6dsox_9dof) or (USING_LSM6DSOX_6DOF and sensor_status.lsm6dsox_6dof)) {
              html = web_pages[PAGE_IMU_ID].html;
              date_time = timestamp(mux, rtc, web_pages[PAGE_IMU_ID].name, SHOW_FULL_DATE, SHOW_12_HOURS, SHOW_LONG_DATE, SHOW_SECONDS);
