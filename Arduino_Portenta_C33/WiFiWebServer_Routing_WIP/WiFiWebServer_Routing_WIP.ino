...
void loop(void) {
  Environment_Data sensors;
  float celsius, fahrenheit, humidity, lux;
  WiFiClient client;
  int page_id = 0;
  String HTTP_req = "", html, temperature_text;
...
*/
    ...
    if (HTTP_req.indexOf("GET") == 0) {  // check if request method is GET
      if (HTTP_req.indexOf("GET / ") > -1 || HTTP_req.indexOf("GET /index ") > -1 || HTTP_req.indexOf("GET /index.html ") > -1) {
        ...
        page_id = PAGE_HOME;
      } else if (HTTP_req.indexOf("GET /environment ") > -1 || HTTP_req.indexOf("GET /temperature.html ") > -1) {
    ...
    switch (page_id) {
      ...
      case PAGE_ENVIRONMENT:
        ...
        //  Replace the markers by a real values
        html.replace("TEMPERATURE_MARKER", "<span style=\"color: green;\">" + String(fahrenheit) + "&deg;F </span>(<span style=\"color: yellow;\">" + String(celsius) += "&deg;C</span>)");
        html.replace("HUMIDITY_MARKER", "<span style=\"color: magenta;\">" + String(humidity) +"% rH</span>");
        break;
      case PAGE_DOOR:
