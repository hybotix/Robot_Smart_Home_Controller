...
void setup(void) {
  ...
  available = ens160.begin();
 
  if (available) {
    available = ens160.available();
    ...
    if (available) {
      ...
      available = ens160.setMode(ENS160_OPMODE_STD);
    }
  }

  if (available) {
    Serial.println("The ENS160 MOX sensor is available");
  } else {
    Serial.println("Could not initialize the ENS160 MOX sensor!");
